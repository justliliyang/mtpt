...
if ($action == "post")
{
	...
	{
		if ($hassubject){
			sql_query("UPDATE topics SET subject=".sqlesc($subject).",onlyauthor=".sqlesc($onlyauthor)." WHERE id=".sqlesc($topicid)) or sqlerr(__FILE__, __LINE__);
	...
	else
	{
		...
		if ($type == 'new'){ //new topic
			...
			//---- Create topic
			sql_query("INSERT INTO topics (userid, forumid, subject, onlyauthor) VALUES($userid, $forumid, ".sqlesc($subject).",$onlyauthor)") or sqlerr(__FILE__, __LINE__);
...
<?
	...
	if($onlyauthor && 
	$CURUSER['id'] != $author &&
	$CURUSER['id'] != $arr["userid"] &&
	($page || $postsnumber)
	){
		$body = $lang_forums['text_onlyauthor'];
		if($CURUSER['class'] >= 15){
...
if ($action == "viewforum")
{
	...
	if ($numtopics > 0)
	{
		...
		while ($topicarr = mysql_fetch_assoc($topicsres))
		{
			...
			$onlyauthor = $topicarr["onlyauthor"];
...
if ($action == "viewunread")
{
	...
	$maxresults = 25;
	$res = sql_query("SELECT id, userid, forumid, subject, lastpost, hlcolor FROM topics WHERE lastpost > ".$CURUSER['last_catchup'].($beforepostid ? " AND lastpost < ".sqlesc($beforepostid) : "")." ORDER BY lastpost DESC LIMIT 100") or sqlerr(__FILE__, __LINE__);
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		if ($n == 1)
		{
			...
			print("<tr><td class=\"colhead\" align=\"left\">".$lang_forums['col_topic']."</td><td class=\"colhead\" align=\"left\">".$lang_forums['col_author']."</td><td class=\"colhead\" align=\"left\">".$lang_forums['col_forum']."</td></tr>\n");
		}
			$fpuserid = 0 + $arr["userid"];
			$fpauthor = get_username($arr["userid"]);
		...
		"</a></td></tr></table></td><td class\"rowfollow\" align=\"left\">".$fpauthor."</td><td class=\"rowfollow\" align=\"left\"><a href=\"".htmlspecialchars("?action=viewforum&forumid=".$forumid)."\"><b>".$forumname."</b></a></td></tr>\n");
	}
	if ($n > 0)
	{
		print("</table>\n");
...
if ($action == "viewmyunread")
{
	$userid = $CURUSER['id'];

	$beforepostid = 0+$_GET['beforepostid'];
	$maxresults = 25;
	$res = sql_query("SELECT id, userid, forumid, subject, lastpost, hlcolor FROM topics WHERE userid = ".$CURUSER['id']." and lastpost > ".$CURUSER['last_catchup'].($beforepostid ? " AND lastpost < ".sqlesc($beforepostid) : "")." ORDER BY lastpost DESC LIMIT 100") or sqlerr(__FILE__, __LINE__);
...
print("<p align=\"center\"><a href=\"?action=search\"><b>".$lang_forums['text_search']."</b></a> | <a href=\"?action=viewunread\"><b>".$lang_forums['text_view_unread']."</b></a> | <a href=\"?action=viewmyunread\"><b><font color=\"#AA0000\">".$lang_forums['text_view_myunread']."</font></b></a> | <a href=\"?catchup=1\"><b>".$lang_forums['text_catch_up']."</b></a> ".(get_user_class() >= $forummanage_class ? "| <a href=\"forummanage.php\"><b>".$lang_forums['text_forum_manager']."</b></a>":"")."</p>");
