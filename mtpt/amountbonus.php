...
/*	if ( $seedbonus == 0 )
	...
	//print_r( $arr );
	if ( $seedbonus > 0 )
		$bonuschangedmsg = "您的麦粒被增加了[color=red][b]".abs($seedbonus)."[/b][/color]个，";
	elseif ( $seedbonus < 0 )
		$bonuschangedmsg = "您的麦粒被减少了[color=red][b]".abs($seedbonus)."[/b][/color]个，";
	else
		$bonuschangedmsg = "麦粒好像没有变化，今天不会是4月1日吧？！";
	if($reason_1){
		$bonuschangedmsg .= chr(10).chr(10)."原因是：".$reason_1 ;
	}
		$bonuschangedmsg .= chr(10).chr(10)."操作人：".$CURUSER["username"];
