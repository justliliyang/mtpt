...
function get_position_name($position)
{
	...
	switch ($position)
	{
		...
		case 'popup': 
			...
			break;
		case 'shoutindex': 
			$name = $lang_admanage['text_shout_index'];
			break;
		case 'shoutlogin': 
			$name = $lang_admanage['text_shout_login'];
			break;
	}
	return $name;
}
...
function print_ad_editor($position, $row = "")
{
	...
	switch ($position)
	{
		...
		case 'popup': 
			...
			break;
		case 'shoutindex': 
			$note = $lang_admanage['text_shout_index_note'];
			break;
		case 'shoutlogin': 
			$note = $lang_admanage['text_shout_login_note'];
			break;
	}
	if ($row)
	{
...
elseif ($action == 'add')
{
	...
	$validpos = array('header', 'footer', 'belownav', 'belowsearchbox', 'torrentdetail', 'comment', 'interoverforums', 'forumpost', 'popup', 'shoutindex', 'shoutlogin');
...
{
	$position = $_GET['position'];
	$validpos = array('header', 'footer', 'belownav', 'belowsearchbox', 'torrentdetail', 'comment', 'interoverforums', 'forumpost', 'popup', 'shoutlogin', 'shoutindex');
	if (!in_array($position, $validpos))
		stderr($lang_admanage['std_error'], $lang_admanage['std_invalid_position']);
	else
	{
...
$code= "<object  classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0\"  width=\"".$_POST['ad']['flash']['width']."\" height=\"".$_POST['ad']['flash']['height']."\"><param name=\"movie\" value=\"".htmlspecialchars($_POST['ad']['flash']['url'])."\" /><param name=\"quality\" value=\"high\" /><embed src=\"".htmlspecialchars($_POST['ad']['flash']['url'])."\" width=\"".$_POST['ad']['flash']['width']."\" height=\"".$_POST['ad']['flash']['height']."\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\"  type=\"application/x-shockwave-flash\" wmode=\"transparent\"></embed></object>";				
break;
		}
		if ($_POST['isedit']){
...
<li><a href="?action=add&amp;position=shoutindex"><?php echo $lang_admanage['text_shout_index']?></a></li>
<li><a href="?action=add&amp;position=shoutlogin"><?php echo $lang_admanage['text_shout_login']?></a></li>
</ul>
</div>
</span>
</div>
