...
function latteryOnclicked()
{
	...
	if(num1<1||num1>12||num2<1||num2>12||num3<1||num3>12||num4<1||num4>12||num5<1||num5>12||multiple<=0)
	{
		alert("数字输入似乎有错误");
		return false;
	}
...
print("<h2>彩票5/12（12选5）</h2><br/>上期开奖：".$row['num1']."，".$row['num2']." ，".$row['num3']." ，".$row['num4']." ，".$row['num5'] ."<h1><a href=\"lottery.php?action=drawlog\" target=\"_blank\" >往期开奖记录</a></h1>
<table>
...
<form action=\"?action=encash\" method=\"post\">
	...
	if($action == "takelattery")
	{
		...
		if($num1<0||$num1>12||$num2<0||$num2>12||$num3<0||$num3>12||$num4<0||$num4>12||$num5<0||$num5>12||$multiple<=0)
		{
