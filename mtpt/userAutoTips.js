...
var listHTML = '<li><a title="$NAME$" rel="$ID$" >@$NAME$</a></li>';
...
var AutoTips = function(A){
	...
	_this.showList = function(char){
		...
		for(var i=0; i<len; i++){
			...
			//'<li><a title="$ACCOUNT$" rel="$ID$" >$NAME$(@$SACCOUNT$)</a></li>';
			h += html.replace(/\$ACCOUNT\$|\$NAME\$/g,data[i]['username']).replace('$ID$',"[@"+data[i]['username']+"]");
		}
		...
		_this.show();
		selectList.setSelected(0);//默认选择第一项
	...
	_this.enter = function(){
		var tmpRel = selectList.list[selectList.index].getElementsByTagName('A')[0];
		
		if(tmpRel.title == '最多可以@十个用户' )//不理会提示信息
		{
			_this.hide();
			return false;
		}
		if(tmpRel.title.indexOf('查无此人') == 0 )//不理会查无此人信息
		{
			_this.hide();
			return false;
		}
		TT.del(elem, key.length+1, key);
		TT.add(elem, tmpRel.rel + ' ');
		_this.hide();
		return false;
	}
	return _this;
