...
switch($id){
	case '401'://电影
		$hint = "[国别][年份][影片中文名][英文全名][电影类型][字幕情况][格式][其他信息]";
		...
		$html .= '<label class="subcat">类型:&nbsp;&nbsp;&nbsp;&nbsp;</label>' . generateInput($videoType, "checkbox", "movie_type") . '<br/>';
		$html .= '<label class="subcat">语言:&nbsp;&nbsp;&nbsp;&nbsp;</label>' . generateInput($language, "checkbox", "language") . '<br/>';
		...
		break;
	case '402'://剧集
		$hint = "[国别][年份][文件名/英文名][S季度E集数][语言字幕][格式][完结/连载][其他]";
		$html .= '<label class="subcat">国别:&nbsp;&nbsp;&nbsp;&nbsp;</label>' . generateInput($country, "radio", "country") . '<br/>';
		$html .= '<label class="subcat">发行时间:</label><input type="text" name="release_time"/><br/>';
		$html .= '<label class="subcat">中文片名:</label><input type="text" name="chinese_name" style="width:50%"/><br/>';
...
$html .="<b style='color:red;font-size:22px'>标题预览</b><br/><b style='color:blue'>你也可以不使用上面的标题生成系统，直接修改下面的标题。但是如果你这么做，不符合标题格式的种子将被直接移入回收站（没有回收站就删掉）</b><br/>你也可以先自动生成再编辑，但是不要先编辑再点上面的按钮。会覆盖已经编辑的标题。";
$result[] = $hint;
$result[] = $html . "</div>";
